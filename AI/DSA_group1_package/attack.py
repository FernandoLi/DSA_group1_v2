def attack(stat, storage, last_path=None):  # last_path是之前决策的路径
    ...
    def field_compare(me, enemy, stat, storage):
        ...
        for i in range(storage['size'][0]):
            for j in range(storage['size'][1]):
                if stat['fields'][i][j] == me:
                    count[me - 1] = count[me - 1] + 1
                elif stat['fields'][i][j] == enemy:
    ...
    r = field_compare(stat['me']['id'], stat['enemy']['id'], stat, storage)  # 判断双方领地大小
    abb = abs(stat['me']['x'] - stat['enemy']['x'])
    abl = abs(stat['me']['y'] - stat['enemy']['y'])
    if stat['me']['x'] < stat['enemy']['x']:
        x = stat['me']['x']
        east = 1  # 表示对方在自己的东边
    elif stat['me']['x'] > stat['enemy']['x']:
        x = stat['enemy']['x']
        west = 1  # 表示对方在自己的西边
    else:
        x = stat['me']['x']
        east = 2
        west = 2
    if stat['me']['y'] < stat['enemy']['y']:
        y = stat['me']['y']
        south = 1  # 表示对方在自己的南边
    elif stat['me']['y'] > stat['enemy']['y']:
        y = stat['enemy']['y']
        north = 1  # 表示对方在自己的北边
    else:
        y = stat['me']['y']
    ...
    yy = stat['me']['y']
    pas = []  # 记录那些方向能走，那些不能
    if yy - 1 >= 0 and stat['bands'][xx][yy - 1] != stat['me']['id']:  # 北边能不能走
        pas.append(1)
    else:
        pas.append(0)
    if yy + 1 < storage['size'][1] and stat['bands'][xx][yy + 1] != stat['me']['id']:  # 南边能不能走
        pas.append(1)
    else:
        pas.append(0)
    if xx - 1 >= 0 and stat['bands'][xx - 1][yy] != stat['me']['id']:  # 西边能不能走
        pas.append(1)
    else:
        pas.append(0)
    if xx + 1 < storage['size'][0] and stat['bands'][xx + 1][yy] != stat['me']['id']:  # 东边能不能走
        pas.append(1)
    else:
        pas.append(0)
    # 先检查周围有没有能撞的
    if yy - 1 >= 0 and stat['bands'][xx][yy - 1] == stat['enemy']['id'] or (stat['enemy']['x'] == xx and stat['enemy']['y'] == yy - 1):
        optim_path = exchange(stat['me']['direction'], 0, -1)
    elif yy + 1 < storage['size'][1] and stat['bands'][xx][yy + 1] == stat['enemy']['id'] or (stat['enemy']['x'] == xx and stat['enemy']['y'] == yy + 1):
        optim_path = exchange(stat['me']['direction'], 0, 1)
    elif xx - 1 >= 0 and stat['bands'][xx - 1][yy] == stat['enemy']['id'] or (stat['enemy']['x'] == xx - 1 and stat['enemy']['y'] == yy):
        optim_path = exchange(stat['me']['direction'], -1, 0)
    elif xx + 1 < storage['size'][0] and stat['bands'][xx + 1][yy] == stat['enemy']['id'] or (stat['enemy']['x'] == xx + 1 and stat['enemy']['y'] == yy):
        optim_path = exchange(stat['me']['direction'], 1, 0)
    elif (abb == 0 or abl == 0) and psf['dis'] > abb + abl:  # 两方带头处于同一直线，对方无法在被撞到之前回到领地
        flag = 0  # 判断两带头中是不是没有己方纸带
        if abb == 0:
            for i in range(abl):
                if stat['bands'][x][y + i] == stat['me']['id']:
                    flag = 1
                    break
        elif abl == 0:
            for i in range(abb):
                if stat['bands'][x + i][y] == stat['me']['id']:
                    flag = 1
                    break
        ...
        if yy < stat['enemy']['y']:
            ...
            yyy = yy - stat['enemy']['y']
        target = [stat['enemy']['x'], stat['enemy']['y']]  # 初始目标设为对方带头
        mind = [abl + abb, 0]  # 记录最短距离
        for i in range(abb + 1):
            for j in range(abl + 1):
                if stat['fields'][x + i][y + j] == stat['enemy']['id']:
                    locfield[i][j] = 1  # 优先度很低，尽量避免进攻路线穿过对方领地
                elif stat['fields'][x + i][y + j] == stat['me']['id']:
                    locfield[i][j] = 3  # 让进攻路线穿过自己的领地是一种保险的做法
                elif stat['bands'][x + i][y + j] == stat['me']['id']:
                    locfield[i][j] = 0  # 优先度最低，一定不能撞上自己的纸带
                elif stat['bands'][x + i][y + j] == stat['enemy']['id']:
                ...
                if xxx == optim_target[k][0]:  # 在同一横线上
                    ...
                    if band_ex != 1:
                        if field_ex == 0 or (field_ex == 1 and mind[1] == 1):
                            ...
                            if stat['me']['direction'] == 0:  # 当前方向向东
                                if xd == 1 and xx + 1 < storage['size'][0] and stat['bands'][xx + 1][yy] != stat['me']['id']:
                                    optim_path = MIDDLE
                                elif yy - 1 >= 0 and stat['bands'][xx][yy - 1] != stat['me']['id']:
                                    optim_path = LEFT
                                elif yy + 1 < storage['size'][1] and stat['bands'][xx][yy + 1] != stat['me']['id']:
                                    optim_path = RIGHT
                            elif stat['me']['direction'] == 1:  # 当前方向南
                                if xd == 1 and xx + 1 < storage['size'][0] and stat['bands'][xx + 1][yy] != stat['me']['id']:
                                    optim_path = LEFT
                                elif xd == -1 and xx - 1 >= 0 and stat['bands'][xx - 1][yy] != stat['me']['id']:
                                    optim_path = RIGHT
                                elif yy + 1 < storage['size'][1] and stat['bands'][xx][yy + 1] != stat['me']['id']:
                                    optim_path = MIDDLE
                            elif stat['me']['direction'] == 2:  # 当前方向西
                                if xd == -1 and xx - 1 >= 0 and stat['bands'][xx - 1][yy] != stat['me']['id']:
                                    optim_path = MIDDLE
                                elif yy - 1 >= 0 and stat['bands'][xx][yy - 1] != stat['me']['id']:
                                    optim_path = RIGHT
                                elif yy + 1 < storage['size'][1] and stat['bands'][xx][yy + 1] != stat['me']['id']:
                                    optim_path = LEFT
                            elif stat['me']['direction'] == 3:  # 当前方向北
                                if xd == 1 and xx + 1 < storage['size'][0] and stat['bands'][xx + 1][yy] != stat['me']['id']:
                                    optim_path = RIGHT
                                elif xd == -1 and xx - 1 >= 0 and stat['bands'][xx - 1][yy] != stat['me']['id']:
                                    optim_path = LEFT
                    ...
                    if band_ex != 1:
                        if field_ex == 0 or (field_ex == 1 and mind[1] == 1):
                            ...
                            if stat['me']['direction'] == 0:  # 当前方向向东
                                if yd == 1 and stat['bands'][xx][yy + 1] != stat['me']['id']:
                                    optim_path = RIGHT
                                elif yd == -1 and stat['bands'][xx][yy - 1] != stat['me']['id']:
                                    optim_path = LEFT
                                elif xx + 1 < storage['size'][0] and stat['bands'][xx + 1][yy] != stat['me']['id']:
                                    optim_path = MIDDLE
                            elif stat['me']['direction'] == 1:  # 当前方向南
                                if yd == 1 and stat['bands'][xx][yy + 1] != stat['me']['id']:
                                    optim_path = MIDDLE
                                    ...
                                    optim_path = LEFT
                                elif xx - 1 >= 0 and stat['bands'][xx - 1][yy] != stat['me']['id']:
                                    optim_path = RIGHT
                            elif stat['me']['direction'] == 2:  # 当前方向西
                                if yd == 1 and stat['bands'][xx][yy + 1] != stat['me']['id']:
                                    optim_path = RIGHT
                    ...
                    if flag:  # 表示可以通过，没有己方纸带阻拦
                        target = [optim_target[k][0], optim_target[k][1]]  # 更新进攻目标
                        mind = [abs(stat['me']['x'] - optim_target[k][0]) + abs(stat['me']['y'] - optim_target[k][1]), 0]  # 第二个表示是否经过对方领地
                        if abs(stat['me']['x'] - optim_target[k][0]) < abs(stat['me']['y'] - optim_target[k][1]):  # 横边小于竖边，优先沿横边走
                            if stat['me']['direction'] == 0:  # 当前方向向东
                                if xd == 1 and xx + 1 < storage['size'][0] and stat['bands'][xx + 1][yy] != stat['me']['id']:
                                    optim_path = MIDDLE
                                elif yy - 1 >= 0 and stat['bands'][xx][yy - 1] != stat['me']['id']:
